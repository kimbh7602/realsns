        ...
        <div class="row align-items-stretch">
            ...
            <div class="col-6 col-md-6 col-lg-4" data-aos="fade-up" style="padding: 10px 10px" v-for="con in contents"
                ...
                <div class="d-block photo-item">
                    <!-- 이미지 처리 -->
                    <div class="polaroid"
                        v-if="con.dislike < 5 && !reportMyList.includes(con.contentId) || readContents.includes(con.contentId)">
                    ...
                    <div class="polaroid"
                        v-show="con.dislike > 4 && !readContents.includes(con.contentId) && !reportMyList.includes(con.contentId)">
                    ...
                    <div class="polaroid"
                        v-show="reportMyList.includes(con.contentId) && !readContents.includes(con.contentId)">
                        <div style="width:100%; height:100%">
                            <img :src="con.images[0].imageUrl" alt="Image" class="img-fluid pa blur m-0" />
                                    ...
                                    <div class="content">
                                        ...
                                        <div v-on:click="goDetail(con.contentId)">
                                            ...
                                            <div class="mail-title offset-1 col-9 mt-2 ml-3 pb-"
                                                ...
                                                <p style="color:black; font-size:2em; font-family: loveson;">Dear
                                                    {{ loginId }}</p>
                                            </div>
                                            <div class="mail-message offset-2 pt-0 pb-0 col-8 ellipsis"
                                                style="color:black; font-family: loveson; word-break:break-all;text-align:left;"
                                                v-html="con.contentValue">{{ con.contentValue }}</div>
                                            <div class="col-11 col-offset-1 pt-0 pr-0"
                                                style="color:black; font-family: loveson; word-break:break-all;text-align:right;">
                                                from {{ con.userId }}</div>
                                        </div>
                                        <!-- buttons -->
                                        <div class="mb-4 pb-2 d-flex justify-content-around size content-button">
                                            ...
                                            <div @click="clickFollow(con.userId)" v-if="con.userId !== loginId">
                                                <i class="icon-check"
                                                    v-if="followList.includes(con.userId)">{{con.userId}}</i>
                                                <i class="icon-user-plus" v-else>{{con.userId}}</i>
                                            </div>
                                            <div @click="clickScrap(con.contentId, con.scrapButton)"
                                                v-if="con.userId !== loginId">
                                                <i class="icon-bookmark" v-if="con.scrapButton"></i>
                                                <i class="icon-bookmark-o" v-else></i>
                                            </div>
                                            <div v-if="con.userId !== loginId">
                                                <i class="icon-bell" v-if="reportMyList.includes(con.contentId)"
                                                    @click="cancel(con.contentId)"></i>
                                                <i class="icon-bell-o" v-else id="reportId" data-toggle="modal"
                                                    data-target="#exampleModal"
                                                    @click="sendInfo(con.contentId, con.timestamp)"></i>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <div class="btn-group dropright">
                                <button class="btn btn-whatever btn-sm dropdown-toggle" style="font-size:13px;"
                                    type="button" id="dropdownMenu2" data-toggle="dropdown" data-display="static"
                                    aria-haspopup="true" aria-expanded="false">
                                    신고 사항 선택
                                </button>
                                <div class="dropdown-menu" aria-labelledby="dropdownMenu2">
                                    <button class="dropdown-item" type="button"
                                        @click="sendReport(options[0].op1)">{{ options[0].op1 }}</button>
                                    <button class="dropdown-item" type="button"
                                        @click="sendReport(options[0].op2)">{{ options[0].op2 }}</button>
                                    <button class="dropdown-item" type="button" @click="sendReport(options[0].op3)"
                                        data-toggle="collapse" data-target="#collapseExample" aria-expanded="false"
                                        aria-controls="collapseExample">{{ options[0].op3 }}</button>
                                </div>
                            </div>

                            <div class="collapse" id="collapseExample">
                                <div class="card card-body">
                                    <input type="text" v-model="options[0].op4"
                                        placeholder="기타를 선택하신 분은 신고 내용을 입력해주세요.">
                                </div>
                                <input class="btn btn-outline-info" type="button" value="입력" data-toggle="collapse"
                                    data-target="#collapseExample" aria-expanded="false" aria-controls="collapseExample"
                                    @click="sendReport2(options[0].op4)">
                            </div>
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                            <button type="button" class="btn btn-primary" data-dismiss="modal"
                                @click="clickBell()">신고하기</button>
                        </div>
                    </div>
                </div>
            </div>
                ...
                for (var idx = 0; idx < this.contents.length; idx++) {
                    if (this.contents[idx].contentId == num) {
                        if (this.contents[idx].likeButton == false) {
                            ...
                            if (this.contents[idx].userId !== this.loginId && this.followList.includes(this.contents[
                                    idx].userId) == false) {
            ...
            clickFollow(user) {
                ...
                if (this.followList.includes(user) == true) {
                    ...
                    for (var j = 0; j < del.length; j++) {
                        const index = this.contents.findIndex(function (item) {
                            return item.contentId === del[j]
                        })
